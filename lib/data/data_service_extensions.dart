...
import 'package:sunny_sdk_core/data/record_data_service.dart';
...
typedef RecordDataServiceWidgetBuilder<X> = Widget Function(X data, RecordDataService<X> service);
...
extension DataServiceBuilder<X> on DataService<X> {
  ...
  Widget buildWithContext({DataServiceWidgetBuilderWithContext<X> builder, String key, bool allowNull = false}) {
    final service = this;
    return StreamBuilder<X>(
      key: Key("${X}${key ?? 'StreamBuilder'}"),
      stream: service.stream,
      initialData: service.currentValue,
      builder: (context, snapshot) => snapshot.render(context, allowNull: allowNull, successFn: (data) {
...
extension RecordDataServiceBuilder<X> on RecordDataService<X> {
  Widget buildFromRecordStream(String recordId, {RecordDataServiceWidgetBuilder<X> builder, String key}) {
    final service = this;
    assert(recordId != null);
    return StreamBuilder<X>(
      key: Key("${X}${key ?? recordId}"),
      stream: service.recordStream(recordId),
      builder: (context, snapshot) => snapshot.render(context, successFn: (data) {
        return builder(data, service);
      }),
    );
  }
